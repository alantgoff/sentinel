...
For autonomous agent-to-agent payments this is a real gap. Every builder who wants the kit's AUTONOMOUS mode to be safe for agent-to-agent commerce ends up reimplementing the same guard logic outside the kit. We hit it building **[Sentinel](https://github.com/alangoff/sentinel)** ([commit log](https://github.com/alangoff/sentinel/commits/main); shipped for the Bounty 5 Policy Agent track), where the value proposition *is* "counterparty-aware payments" — so we had to build:

- A rule engine: per-counterparty cap, daily limit, rolling velocity window, service allowlist, denylist, hard cap, escalation threshold ([src/plugin/rules.js](https://github.com/alangoff/sentinel/blob/main/src/plugin/rules.js)).
- A reputation scorer that re-verifies HCS-claimed settlements against the mirror node and only counts the verified subset ([src/plugin/reputation.js](https://github.com/alangoff/sentinel/blob/main/src/plugin/reputation.js)).
- A policy combiner that bumps or shrinks the effective autonomous cap based on the score ([src/plugin/policy.js](https://github.com/alangoff/sentinel/blob/main/src/plugin/policy.js)).
